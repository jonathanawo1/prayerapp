...
class CexMonitor(BaseMonitor):
    ...
    async def check(self, url):
        ...
        p = html.lower()
        if "buy" in p and "out of stock" not in p and "no stock" not in p: r["in_stock"] = True
        if "out of stock" in p or "no stock" in p: r["in_stock"] = False
        return r
