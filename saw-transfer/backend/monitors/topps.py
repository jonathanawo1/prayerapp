...
class ToppsMonitor(BaseMonitor):
    SITE_NAME = "topps"
    async def check(self, url):
        ...
        if jld:
            try:
                ...
                if o.get("price"): r["price"] = f"{'£' if cur=='GBP' else '$'}{o['price']}"
                av = o.get("availability", "")
                if "InStock" in av: r["in_stock"] = True
                elif "OutOfStock" in av: r["in_stock"] = False
            except: pass
        p = html.lower()
        if ("add to cart" in p or "add to bag" in p) and "out of stock" not in p: r["in_stock"] = True
