...
class AsdaMonitor(BaseMonitor):
    ...
    async def check(self, url):
        ...
        p = html.lower()
        if "add to trolley" in p or "add to basket" in p: r["in_stock"] = True
        if "out of stock" in p or "temporarily unavailable" in p: r["in_stock"] = False
        return r
